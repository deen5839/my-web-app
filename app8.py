...
with st.sidebar:
    st.header("⚙️ 參數設定")
    ticker = st.text_input("股票代號 (台股請加 .TW)", value="5706.TW")
    ...
    # 💡 新增：特例修正勾選框 (專門處理像鳳凰 5706 這種 Yahoo 舊資料算錯的股票)
    enable_custom_history = st.checkbox("啟用舊年份真實股息覆蓋 (修復 API 誤植)", value=False)
...
with st.spinner(f"📡 正在從全球金融資料庫撈取 {ticker} 的歷史配息..."):
    try:
        ...
        if div_data.empty and not enable_custom_history:
            st.warning(f"⚠️ 找不到 {ticker} 的配息資料，請檢查代號是否正確。")
        else:
            # 整理 Yahoo Finance 資料
            df_div = pd.DataFrame(div_data).reset_index() if not div_data.empty else pd.DataFrame(columns=['Date', 'Dividends'])
            if not df_div.empty:
            ...
            # 💡 特例修復：若勾選「啟用舊年份真實股息覆蓋」，針對鳳凰等特例直接覆蓋玩股網的真實歷史現金配息
            if enable_custom_history and ticker == "5706.TW":
                history_fix = {2021: 0.30, 2022: 1.00, 2023: 0.30, 2024: 2.00, 2025: 2.75, 2026: 4.50}
                for yr, val in history_fix.items():
            ...
            # 未來年份 (> 2026) 延續最新股息推算
            latest_div = yearly_div.loc[yearly_div['Year'] == current_year, 'Dividends'].values[0] if current_year in yearly_div['Year'].values else manual_2026_div
            future_mask = yearly_div['Year'] > current_year
            yearly_div.loc[future_mask, 'Dividends'] = latest_div if latest_div > 0 else manual_2026_div
